...
This is not an optimization loop. The allowed sequence is one preflight, one launch, and at
most one automatic quota fallback. Keep the fallback only when the terminal reaches
`tui-idle` and the task is successfully delivered; otherwise retain the original failure and
stop.

...
description: >-
  ...
  Orca 터미널의 코딩 에이전트 작업에 이 스킬을 사용합니다. Orca가 아는 에이전트에만
  `--agent` 경로를 쓰고, OMO·GJC 같은 미등록 CLI는 터미널로 실행하며 검증된
  모델/thinking 선택과 쿼터 인지 복구를 적용합니다. 이미 열린 미등록 에이전트 탭은
  native `worker-start` 워커가 아닌 custom-dispatch 워커입니다. 감독 없는 전체 위임, 일반
...
> Orca의 감독형 워커를 안전하게 조정하고, `--agent` provider로 등록되지 않은 코딩 에이전트도
> 모델 인지 방식으로 실행합니다.
...
Orca가 인식하는 에이전트는 `worker-start`로 native supervised worker가 될 수 있습니다. OMO 같은
미등록 CLI 탭은 **custom-dispatch 워커**입니다. Orca는 Run, Task, Dispatch, 터미널 출력을
추적하고, 코디네이터는 정확한 Dispatch preamble과 task spec을 수동 전달합니다. 그 전달 뒤에는
Dispatch lifecycle이 권한을 갖지만 기존 terminal은 사용자 소유입니다. 이 경로를 native
`worker-start` worker라고 부르거나 terminal 생성만으로 delivery를 주장하지 않습니다.
...
| Scope | Orca/에이전트 CLI 도움말과 로컬 준비 상태를 확인하고, 허가된 워크트리/터미널을 만들며, 선언한 작업을 보낼 수 있습니다. 기본적으로 워커는 orchestration을 시작한 것과 같은 코딩 에이전트를 사용합니다. 기존 미등록 에이전트 탭은 수동 exact-preamble delivery가 있는 custom Dispatch만 사용합니다. 동의 없이 게시, 배포, credential 공개, 모델 기본값 영구 저장은 하지 않습니다. |
...
| Verification | 실행 전에 명령 기능을 확인하고, 최초 텍스트 전송 전 `tui-idle`을 기다리며, 실제 터미널/결과 상태를 검사하고, 사용자의 명시 선택을 보존합니다. custom-dispatch 워커는 Dispatch를 확인하고 exact preamble을 가져오며 prompt delivery를 확인한 뒤 Dispatch lifecycle 메시지를 기다립니다. |
...
3. **워커 종류 선택.** Run을 만들거나 bind하고 Task를 만든 뒤 Orca가 아는 native supervised
   agent에만 `worker-start --task <task-id> --agent <registered-agent>`를 사용합니다.
   `worker-start`만 native worktree/terminal 생성, task 전달, launch lifecycle을 소유하며 같은
   native worker를 `worktree create --agent`로 미리 만들지 않습니다. `worker_done`, `escalation`,
   `question`을 기다립니다. 미등록 CLI는
   `rules/agent-selection.ko.md`의 custom-dispatch 경로를 사용합니다. CLI를 검증하고 기존 eligible
   terminal이 없을 때만 terminal을 만들며, `tui-idle`을 기다리고, `--inject` 없이 low-level
   Dispatch를 만들고, `dispatch-show --preamble`을 가져온 뒤 정확한 preamble과 task spec을 하나의
   prompt로 보냅니다. `worker-start --agent omo`를 호출하거나
   `worker-start --terminal <omo-handle>`가 된다고 가정하지 않습니다. 어느 쪽도 OMO를 등록된
   native worker로 만들지 않습니다.
...
6. **비용 없는 준비 상태 확인.** 가능한 모델, credential, 과거 사용량 신호를 수집합니다.
   ...
   보내지 않습니다.
7. **실행, 대기, 전달.** 새 워커는 돌려준 핸들 하나만 유지합니다. 기존 OMO 워커는 사용자 소유
   handle을 유지하고 `terminal show`, `tui-idle` 대기를 사용하며 replacement tab을 만들지
   않습니다. 모든 custom-dispatch 워커는 Task/terminal을 1:1로 매핑하고, `--inject` 없이
   Dispatch를 만들고, 정확한 `dispatch-show --preamble` 응답을 가져오며, 그 원문 preamble과
   task spec을 보내고 delivery를 확인한 뒤에만 worker를 active로 표시합니다. exact preamble을
   보낸 뒤에는 `worker_done`, `escalation`, `question`을 기다리고 돌려받은 ID/명령을 추측 없이
   사용합니다. 요청 시 의미 있는 시점에 Orca task/worktree 상태를 갱신합니다.
...
- 사용자가 다른 워커 에이전트를 명시하지 않는 한 모든 기본 워커에서 원본 코딩 에이전트를
  보존합니다.
- 기존 OMO 또는 다른 미등록 CLI 탭은 custom-dispatch 워커로 분류하고, 사용자 소유 terminal을
...
- 미등록 OMO terminal임을 안 뒤 `worker-start --agent omo`,
  `worker-start --terminal <omo-handle>`, `dispatch --inject`를 기존 OMO terminal을 등록하는
  방법이라고 가정해 호출하는 행위

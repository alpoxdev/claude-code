...
- baseline은 알려진 정적 finding과 이후 scan을 비교하는 저장된 `detect-slop.cjs` v2 JSON 결과다.
...
optional `.ai-slop-remover.json`은 승인된 값 하나 또는 파일 하나의 rule을 기록할 수 있다. 의존하기 전에 `scripts/validate-waivers.cjs`로 검증한다.

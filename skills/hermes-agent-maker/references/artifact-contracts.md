...
> Sources: [Hermes context files](https://hermes-agent.nousresearch.com/docs/user-guide/features/context-files), [personality](https://hermes-agent.nousresearch.com/docs/user-guide/features/personality), [memory](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory), [plugins](https://hermes-agent.nousresearch.com/docs/user-guide/features/plugins), and [plugin authoring](https://hermes-agent.nousresearch.com/docs/developer-guide/plugins). Local evidence: `instructions/cli/hermes-agent/`.

Use one `kind` only: `skill`, `native-plugin`, `portable-plugin`, `soul`, `agents`, `user-draft`, or `memory-draft`. Interpret requests in the skill; generators receive only normalized data and static local templates. Every preview lists the complete ordered change-set and current preimages; it writes nothing. Apply requires approval bound to that exact preview.

## Output contracts

| Kind | Required output | Boundary |
| --- | --- | --- |
| `skill` | A target directory containing paired `SKILL.md` and `SKILL.ko.md`; add paired resources only when justified. | Instruction/workflow package, not executable integration. Preserve required Agent Skills frontmatter (`name`, `description`). |
...
| `portable-plugin` | A target directory with `plugin.json`, `skills/<name>/SKILL.md`, and optional `mcp.json`. | Validate against local pinned Agent Plugins v1.0.0 material first, then the Hermes subset in `portable-agent-plugins-v1.md`. It is not a native Python plugin. |
...
`plugin.yaml` declares identity/version and accurately declares provided tools/hooks. `__init__.py` exposes `register(ctx)`. Register tools with documented `ctx.register_tool(...)`, hooks with `ctx.register_hook(...)`, and packaged skills with `ctx.register_skill` only when present. Keep imports and registration deterministic; defer credential reads, network access, background work, and mutations to handlers. Use documented APIs, accept additive hook keyword arguments, and never silently approve actions.

Model-facing schemas state purpose, inputs, and limits. Handlers validate untrusted arguments, bound work, redact sensitive output, catch expected errors, and do not use `eval` on model or user input. Native capabilities and required environment variables must be explicit; missing required variables disable the plugin. Declared Python dependencies are not auto-installed.
...
1. Route to exactly one kind; for an unspecified plugin kind, explain native versus portable and collect that one decision in easy Korean.
2. Produce English/Korean pairs for user-facing package documents.
3. Preview all creates, updates, deletions, modes, hashes, and preimages without writing.
4. Require identity-bound approval before apply; `user-draft` and `memory-draft` remain drafts after approval.
